...
'        Console.WriteLine($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.ffffff")} 【{invocation.Method.Name} 】开始 : ")
        ...
        try
            invocation.Proceed()
        Catch ex As Exception
            Console.WriteLine($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.ffffff")} 【{invocation.Method.Name} 】异常 : {ex.Message} {ex.StackTrace}: {JsonConvert.SerializeObject(PrintParameters(invocation))}")
        End Try
