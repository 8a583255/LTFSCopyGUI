...
'            Console.WriteLine(($" New FileDesc GetSecurityByName： {FileName} {filedesc.OperationId}"))
        ...
        For i As Integer = 0 To path.Length - 2
            ...
            For Each d As ltfsindex.directory In LTFSDir.contents._directory
                ...
                End If
            Next
            If Not dirFound Then Return STATUS_OBJECT_NAME_NOT_FOUND
        Next
        For Each d As ltfsindex.directory In LTFSDir.contents._directory
            If d.name = path(path.Length - 1) Then
                FileExist = True
                filedesc = New FileDesc _
                    With {.IsDirectory = True, .LTFSDirectory = d, .OperationId = Guid.NewGuid().ToString()}
'                Console.WriteLine(($" New FileDesc GetSecurityByName： {FileName} {filedesc.OperationId}"))
                Exit For
            End If
        Next
        If Not FileExist Then
            For Each f As ltfsindex.file In LTFSDir.contents._file
                If f.name = path(path.Length - 1) Then
                    FileExist = True
                    filedesc = New FileDesc _
                        With {.IsDirectory = False, .LTFSFile = f, .Parent = LTFSDir,
                            .OperationId = Guid.NewGuid().ToString()}
'                    Console.WriteLine(($" New FileDesc GetSecurityByName： {FileName} {filedesc.OperationId}"))
                    Exit For
                End If
            Next
            For Each f As ltfsindex.file In LTFSDir.contents.UnwrittenFiles
                If f.name = path(path.Length - 1) Then
                    FileExist = True
                    filedesc = New FileDesc _
                        With {.IsDirectory = False, .LTFSFile = f, .Parent = LTFSDir,
...
'                    Console.WriteLine(($" New FileDesc GetSecurityByName： {FileName} {filedesc.OperationId}"))
    ...
    Public Shared CurrentWriteFileCount As Integer
...
'        Console.WriteLine($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.ffffff")}  ReadDirectoryEntry() FileNode：{JsonConvert.SerializeObject(FileNode)} FileDesc0:{JsonConvert.SerializeObject(FileDesc0)} Pattern:{Pattern} Marker:{Marker}")
        ...
        If FileDesc Is Nothing OrElse FileDesc.FileSystemInfos Is Nothing Then
            ...
            For Each f As ltfsindex.file In FileDesc.LTFSDirectory.contents._file
                ...
                End If
            Next
            For Each f As ltfsindex.file In FileDesc.LTFSDirectory.contents.UnwrittenFiles
                If f.name.ToLower() like Pattern.ToLower() Then
                    lst.Add(f.name, f)
                End If
            Next
...
'        Console.WriteLine($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.ffffff")}  【Create】 {fileName} createOptions {createOptions} grantedAccess {grantedAccess} fileAttributes {fileAttributes} allocationSize {allocationSize}")
        ...
        For Each d As ltfsindex.directory In LTFSDir.contents._directory
            If d.name = path(path.Length - 1) Then
                FileExist = True
                Console.WriteLine($"文件夹名字冲突：{LTFSDir.name}下的{d.name}")
                Return STATUS_OBJECT_NAME_COLLISION
            End If
        Next
        For Each d As ltfsindex.file In LTFSDir.contents._file
            If d.name = path(path.Length - 1) Then
                FileExist = True
                Console.WriteLine($"文件名字冲突：{LTFSDir.name}下的{d.name}")
                Return STATUS_OBJECT_NAME_COLLISION
            End If
        Next
...
'                            End If
    ...
    Public Overrides Function OverwriteEx(ByVal fileNode As Object, ByVal fileDesc0 As Object,
        ...
        If FileDesc IsNot Nothing Then
            fileDesc_FileName = FileDesc.LTFSFile.name
        End If
        FileDesc.Parent.contents._file.Remove(FileDesc.LTFSFile)
        Dim fileRecord = New LTFSWriter.FileRecord(FileDesc.LTFSFile.name, 0, "", FileDesc.Parent)
        '                fileDesc = New FileDescriptor(fileName, path, New MemoryStream(), True)
        '                fileDesc.SetFileAttributes(fileAttributes Or CUInt(System.IO.FileAttributes.Archive))
...
'    End If
        ...
        if NOT fileDesc.UnwriteFile is Nothing then
            ...
            FileDesc.Parent.contents.UnwrittenFiles.Remove(FileDesc.LTFSFile)
